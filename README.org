...
Once installed, on the first call to =(require 'zmq)= you will be asked to
build the C module dependency. If you would prefer not to attempt to build the
C module, pre-built binaries are available for the most common OS's at the
[[https://github.com/dzop/emacs-zmq/releases][releases]] page.
